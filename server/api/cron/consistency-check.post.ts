...
interface InconsistencyReport {
  ...
  type: 'status_mismatch' | 'missing_payment' | 'amount_mismatch' | 'stale_authorization'
...
レポートID: ${reportId}
    ...
    await systemLogger.consistencyCheckCompleted(results.issues, results.autoFixed)
...
function checkStatusConsistency(
...
): { description: string; severity: 'low' | 'medium' | 'high'; autoFix?: Record<string, unknown> } | null {
  ...
  // 自動修復可能なケース
  if (stripeStatus === 'succeeded' && !['confirmed', 'refunded', 'completed'].includes(firestoreStatus)) {
    return {
