...
export default defineEventHandler(async (event) => {
  ...
  // メッセージプレビュー（100文字まで）
  const preview = messagePreview || (messageContent ? messageContent.substring(0, 100) + (messageContent.length > 100 ? '...' : '') : '')
  ...
  if (type === 'guest_to_admin') {
    ...
    throw createError({
      statusCode: 400,
      statusMessage: '無効なtypeです。guest_to_admin または admin_to_guest を指定してください'
    })
  }
  ...
  try {
    ...
    console.log(`✅ Message notification email sent (${type}):`, info.messageId)
