...
async function handleAuthorizationSuccess(
  ...
  try {
    const checkInDate =
      bookingData.checkInDate?.toDate?.() || new Date(bookingData.checkInDate);
    const checkOutDate =
...
async function handlePaymentFailed(
  ...
  try {
    ...
    logger.error(
      "Payment failed email sending failed:",
...
function translateStripeError(code?: string, fallbackMessage?: string): string {
  const errorMessages: Record<string, string> = {
    ...
    authentication_required:
      "3Dセキュア認証が必要です。認証を完了してください。",
    payment_intent_authentication_failure:
      "本人認証に失敗しました。カード会社にお問い合わせください。",
    rate_limit:
      "短時間に多くのリクエストがありました。しばらく経ってから再度お試しください。",
  };
