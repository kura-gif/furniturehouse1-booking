...
interface InconsistencyReport {
  ...
  type: 'status_mismatch' | 'missing_payment' | 'orphan_payment' | 'amount_mismatch' | 'stale_authorization'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (dateFrom) {
      bookingsQuery = bookingsQuery.where('createdAt', '>=', new Date(dateFrom))
    } else {
      // デフォルト: 直近30日
    ...
    for (const bookingDoc of bookingsSnapshot.docs) {
      ...
      try {
        ...
        if (
          ...
          const authorizedAt = booking.authorizedAt?.toDate?.() || new Date(booking.authorizedAt)
        ...
        if (stripeError.code === 'resource_missing') {
          inconsistencies.push({
            ...
            suggestedAction: '手動で確認し、予約をキャンセルするか新規決済を作成してください',
            autoFixable: false
          })
        } else {
    ...
    console.log(`✅ Consistency check completed: ${inconsistencies.length} issues found`)
...
function checkStatusMismatch(
  ...
  if (!validStripeStatuses.includes(stripeStatus)) {
    // 自動修復可能なケース
    if (stripeStatus === 'succeeded' && firestoreStatus !== 'confirmed' && firestoreStatus !== 'refunded') {
      return {
