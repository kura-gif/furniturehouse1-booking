  ...
  try {
    ...
    return {
      ...
      settings: {
        ...
        houseRulesProhibited:
          data?.houseRulesProhibited || defaultSettings.houseRulesProhibited,
        houseRulesNoise:
          data?.houseRulesNoise || defaultSettings.houseRulesNoise,
        houseRulesGarbage:
          data?.houseRulesGarbage || defaultSettings.houseRulesGarbage,
        houseRulesCheckout:
          data?.houseRulesCheckout || defaultSettings.houseRulesCheckout,

        // キャンセルポリシー
        cancelPolicyFree:
          data?.cancelPolicyFree || defaultSettings.cancelPolicyFree,
        cancelPolicyFreeDesc:
          data?.cancelPolicyFreeDesc || defaultSettings.cancelPolicyFreeDesc,
        cancelPolicyPartial:
          data?.cancelPolicyPartial || defaultSettings.cancelPolicyPartial,
        cancelPolicyPartialDesc:
          data?.cancelPolicyPartialDesc ||
          defaultSettings.cancelPolicyPartialDesc,
        cancelPolicyNoShow:
          data?.cancelPolicyNoShow || defaultSettings.cancelPolicyNoShow,
        cancelPolicyNoShowDesc:
          data?.cancelPolicyNoShowDesc ||
          defaultSettings.cancelPolicyNoShowDesc,
        cancelPolicyProcedure:
          data?.cancelPolicyProcedure || defaultSettings.cancelPolicyProcedure,
        cancelPolicyExceptions:
          data?.cancelPolicyExceptions ||
          defaultSettings.cancelPolicyExceptions,
        cancelPolicyNotes:
          data?.cancelPolicyNotes || defaultSettings.cancelPolicyNotes,
        ...
        accessByPublicTransport:
          data?.accessByPublicTransport ||
          defaultSettings.accessByPublicTransport,
        nearbyHospital: data?.nearbyHospital || defaultSettings.nearbyHospital,
      },
    };
  } catch (error: unknown) {
    console.error(
      "[API /public/settings] Error:",
