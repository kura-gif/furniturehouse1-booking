...
import type { Supporter, SupportTask, SupporterAvailability, SupportMessage, SupportDuration, SupportStatus } from '~/types'
...
export const useSupport = () => {
  ...
  // サポーターを作成
  const createSupporter = async (supporterData: Omit<Supporter, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // サポーター情報を更新
  const updateSupporter = async (supporterId: string, data: Partial<Supporter>): Promise<void> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // 予約IDからサポートタスクを取得
  const getSupportTaskByBookingId = async (bookingId: string): Promise<SupportTask | null> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // サポートタスクを作成
  const createSupportTask = async (taskData: Omit<SupportTask, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // サポートタスクを更新
  const updateSupportTask = async (taskId: string, data: Partial<SupportTask>): Promise<void> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // サポーターをタスクに割り当て
  const assignSupporter = async (taskId: string, supporterId: string): Promise<void> => {
    await updateSupportTask(taskId, { supporterId, status: 'scheduled' })
  }

  // タスクを完了にする
  const completeSupportTask = async (taskId: string, actualDuration: number): Promise<void> => {
  ...
  // サポーターの利用可能スケジュールを取得
  const getSupporterAvailability = async (supporterId: string, startDate: Date, endDate: Date): Promise<SupporterAvailability[]> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  const setSupporterAvailability = async (
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        // 既存のスケジュールを更新
        const docRef = doc($db, 'supporterAvailability', querySnapshot.docs[0].id)
  ...
  // タスクのメッセージ一覧を取得
  const getSupportMessages = async (taskId: string): Promise<SupportMessage[]> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // タスクの料金を計算
  const calculateTaskCost = (supporter: Supporter, actualDuration: number): {
    laborCost: number
