...
export const useGuestGuide = () => {
   ...
   */
  const getGuideAccessByToken = async (
    token: string,
  ): Promise<GuestGuideToken | null> => {
   ...
   * 予約IDからトークンを取得
   */
  const getGuideTokenByBookingId = async (
    bookingId: string,
  ): Promise<GuestGuideToken | null> => {
    if (!$db) throw new Error("Database not initialized");
    const tokensRef = collection($db, "guestGuideTokens");
   ...
   * 周辺スポット一覧を取得
   */
  const getAreaSpots = async (
    category?: AreaSpotCategory,
  ): Promise<GuideAreaSpot[]> => {
    if (!$db) throw new Error("Database not initialized");
   ...
   * 同意記録を確認
   */
  const checkRulesAgreement = async (
    bookingId: string,
  ): Promise<GuideRulesAgreement | null> => {
    if (!$db) throw new Error("Database not initialized");
    const agreementsRef = collection($db, "guideRulesAgreements");
