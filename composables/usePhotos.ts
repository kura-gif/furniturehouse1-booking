...
export const usePhotos = () => {
  ...
  // 写真を更新
  const updatePhoto = async (photoId: string, photoData: Partial<Photo>): Promise<void> => {
    if (!$db) throw new Error('Firestore is not initialized')
  ...
  // 写真の公開状態を切り替え
  const togglePhotoVisibility = async (photoId: string, isVisible: boolean): Promise<void> => {
    await updatePhoto(photoId, { isVisible })
  }

  // 写真の順序を更新
  const updatePhotoOrder = async (photoId: string, order: number): Promise<void> => {
