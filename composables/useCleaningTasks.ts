...
export const useCleaningTasks = () => {
  ...
  const createCleaningTasks = async (booking: Booking): Promise<void> => {
    try {
      ...
      const checkInDate =
        (booking as { startDate?: string }).startDate || booking.checkInDate;
      const checkOutDate =
        (booking as { endDate?: string }).endDate || booking.checkOutDate;
  ...
  const loadTasks = async (filters?: {
    ...
    try {
      ...
      if (filters?.startDate) {
        constraints.push(
          where("scheduledDate", ">=", Timestamp.fromDate(filters.startDate)),
        );
      }
      if (filters?.endDate) {
        constraints.push(
          where("scheduledDate", "<=", Timestamp.fromDate(filters.endDate)),
        );
      }
   ...
   */
  const getTasksBySupporter = async (
    supporterId: string,
