  ...
  if (holidayCalendar && holidayCalendar.length > 0) {
    ...
    for (const cal of holidayCalendar) {
      if (cal.holidays && cal.holidays.includes(nextDayStr)) {
        console.log(
          `🎌 祝日前日判定: ${formatDateToYYYYMMDD(date)} の翌日 ${nextDayStr} は祝日です`,
        );
        return "weekend";
      }
    }
 ...
 * 泊数に応じた説明テキストを取得
 */
export function getNightRateDescription(
  nightNumber: number,
  rate: number,
): string {
...
export function calculateGuestCountCharges(
  ...
  // 5人目の料金
  if (totalAdults >= 5) {
    charges.guest5th = Math.floor(
      thirdGuestCharge * (guestCountPricing.additionalGuestRates?.fifth ?? 0.8),
    );
  ...
  // 6人目の料金
  if (totalAdults >= 6) {
    charges.guest6th = Math.floor(
      thirdGuestCharge * (guestCountPricing.additionalGuestRates?.sixth ?? 0.7),
    );
...
export function calculateEnhancedPrice(
  ...
  // デバッグ: 日付を確認
  console.log(
    "[calculatePriceInternal] checkInDate:",
    checkInDate,
    checkInDate?.getTime?.(),
  );
  console.log(
    "[calculatePriceInternal] checkOutDate:",
    checkOutDate,
  ...
  for (let i = 0; i < numberOfNights; i++) {
    ...
    const dayTypeMultiplier =
      pricingSetting.dayTypeMultipliers?.[dayType] ??
    ...
    const nightRateDescription = getNightRateDescription(
      nightNumber,
      nightRate,
    );
  ...
  const averagePricePerPerson =
    totalGuestCount > 0 ? Math.floor(totalAmount / totalGuestCount) : 0;
...
export async function saveEnhancedPricingSettingsToFirestore(
  settings: Partial<Pick<EnhancedPricingSetting, "id">> &
    Omit<EnhancedPricingSetting, "id" | "createdAt" | "updatedAt">,
): Promise<string> {
  const { $db } = useNuxtApp();
  if (!$db) throw new Error("Firestore is not initialized");

  const { collection, addDoc, doc, setDoc, Timestamp } =
    await import("firebase/firestore");
  ...
  if (settings.id) {
    const docRef = doc(
      $db as ReturnType<typeof import("firebase/firestore").getFirestore>,
 ...
 */
export function saveEnhancedPricingSettingsToLocalStorage(
  settings: EnhancedPricingSetting,
): void {
...
export const useEnhancedPricing = () => {
  const pricingSetting = ref<EnhancedPricingSetting>(
    loadEnhancedPricingSettingsFromLocalStorage(),
  );
  ...
  const loadFromFirestore = async () => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "料金設定の読み込みに失敗しました";
    } finally {
      loading.value = false;
    }
  };
  ...
  const saveToFirestore = async (settings: EnhancedPricingSetting) => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "料金設定の保存に失敗しました";
      throw err;
    } finally {
      loading.value = false;
    }
  ...
  const calculatePrice = (
    ...
    try {
      ...
      const weekendMultiplier =
        pricingSetting.value.dayTypePricing?.weekendMultiplier || 1.3;
      const holidayCalendarData = pricingSetting.value.holidayCalendar || [];

      // 祝日データの詳細をログ
      const allHolidays = holidayCalendarData.flatMap(
        (c: { holidays?: string[] }) => c.holidays || [],
      );
      ...
      console.log(
        `📅 ${checkInStr} の翌日 ${nextDayStr} は祝日: ${isNextDayHoliday}`,
      );
