...
export const useConversations = () => {
   ...
   * ゲストの会話一覧を取得
   */
  const getGuestConversations = async (
    guestUserId: string,
  ): Promise<Conversation[]> => {
    if (!$db) throw new Error("Firestore is not initialized");
    const conversationsRef = collection($db, "conversations");
   ...
   */
  const getConversationByBookingId = async (
    bookingId: string,
    guestEmail?: string,
  ): Promise<Conversation | null> => {
    if (!$db) throw new Error("Firestore is not initialized");
  ...
  const sendMessage = async (
    ...
    if (senderType === "admin") {
      ...
      const response = await $fetch<{ success: boolean; messageId: string }>(
        "/api/conversations/admin-send-message",
        {
          method: "POST",
          headers: {
