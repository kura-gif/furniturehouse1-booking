...
export const useMaintenance = () => {
  ...
  const createMaintenanceRecord = async (
    ...
    try {
      ...
      if (recordData.nextScheduledDate)
        cleanData.nextScheduledDate = recordData.nextScheduledDate;
      if (recordData.description)
        cleanData.description = recordData.description;
      if (recordData.performedBy)
        cleanData.performedBy = recordData.performedBy;
  ...
  const updateMaintenanceRecord = async (
    recordId: string,
    recordData: Partial<
      Omit<MaintenanceRecord, "id" | "createdAt" | "updatedAt">
    >,
  ): Promise<void> => {
    if (!$db) throw new Error("Firestore is not initialized");
    ...
    try {
      ...
      if (recordData.equipmentName !== undefined)
        cleanData.equipmentName = recordData.equipmentName;
      if (recordData.lastMaintenanceDate !== undefined)
        cleanData.lastMaintenanceDate = recordData.lastMaintenanceDate;
      if (recordData.nextScheduledDate !== undefined)
        cleanData.nextScheduledDate = recordData.nextScheduledDate;
      if (recordData.status !== undefined) cleanData.status = recordData.status;
      if (recordData.description !== undefined)
        cleanData.description = recordData.description;
      if (recordData.performedBy !== undefined)
        cleanData.performedBy = recordData.performedBy;
  ...
  const logMaintenance = async (
    ...
    const existingRecord = records.find(
      (r) => r.equipmentName === equipmentName,
    );
    ...
    if (existingRecord) {
      await updateMaintenanceRecord(existingRecord.id, {
        ...
        nextScheduledDate: nextScheduledDate
          ? Timestamp.fromDate(nextScheduledDate)
          : undefined,
        status: "good",
        description,
        performedBy,
  ...
  const getRecordsNeedingAttention = async (): Promise<MaintenanceRecord[]> => {
    ...
    return allRecords.filter(
      (r) => r.status === "needs_attention" || r.status === "broken",
    );
  };
