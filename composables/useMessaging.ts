...
export const useMessaging = () => {
  ...
  const sendMessage = async (
    ...
    try {
      ...
      const docRef = await addDoc(collection($db, 'guestMessages'), messageData)
      return docRef.id
    } catch (error) {
   ...
   * 予約の未読メッセージをすべて既読にする
   */
  const markAllMessagesAsRead = async (bookingId: string, currentUserId: string): Promise<void> => {
    if (!$db) throw new Error('Firestore is not initialized')
   ...
   * 予約の未読メッセージ数を取得
   */
  const getUnreadCount = async (bookingId: string, currentUserId: string): Promise<number> => {
    if (!$db) throw new Error('Firestore is not initialized')
