...
export const useInventory = () => {
  ...
  const updateInventoryItem = async (
    ...
    try {
      ...
      if (itemData.currentStock !== undefined) cleanData.currentStock = itemData.currentStock
      if (itemData.unit !== undefined) cleanData.unit = itemData.unit
      if (itemData.reorderThreshold !== undefined) cleanData.reorderThreshold = itemData.reorderThreshold
      if (itemData.purchaseUrl !== undefined) cleanData.purchaseUrl = itemData.purchaseUrl
  ...
  // 在庫数を更新
  const updateStock = async (itemId: string, newStock: number): Promise<void> => {
