...
| **Phase 2.5: 公開前リスク対策** | 🔄 進行中 | **1/18(土)** | **1/19(日)** | - |
...
| # | タスク | リスク | 影響度 | 状態 | 完了日 |
|---|--------|--------|--------|------|--------|
| 2.5-1 | **Stripe idempotencyKey追加** | リトライで二重課金 | 💰 金銭的損害 | ⬜ | |
| 2.5-2 | **console.log機密情報削除** | PII漏洩・ログ肥大化 | 🔒 セキュリティ | ⬜ | |
| 2.5-3 | **エラー詳細マスク** | スタックトレース露出 | 🔒 セキュリティ | ⬜ | |
| 2.5-4 | **環境変数バリデーション** | 未設定でサイレント失敗 | 🚨 本番クラッシュ | ⬜ | |
| 2.5-5 | **Firestore Rules修正** | sentEmailsが`allow create: if true` | 🔒 セキュリティ | ⬜ | |
...
| # | タスク | リスク | 状態 | 完了日 |
|---|--------|--------|------|--------|
| 2.5-6 | **型定義の`any`修正** | 価格計算でランタイムエラー | ⬜ | |
| 2.5-7 | **npm脆弱性再確認** | 13個のHigh脆弱性 | ⬜ | |
| 2.5-8 | **Firestoreバックアップ設定** | データ消失時に復旧不可 | ⬜ | |
| 2.5-9 | **localhostフォールバック削除** | メールリンク壊れる | ⬜ | |
