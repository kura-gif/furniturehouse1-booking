<template>
  <div class="min-h-screen bg-white">
    ...
    <div class="max-w-[1280px] mx-auto px-6 md:px-12 py-8">
      ...
      <div class="mb-8">
        <div
          class="grid grid-cols-4 gap-2 h-[400px] md:h-[500px] rounded-lg overflow-hidden relative"
        >
          <!-- メイン画像 (左側大きく) -->
          <div
            @click="openPhotoTour"
      ...
      <div v-if="showPhotoTour" class="fixed inset-0 z-50 bg-white">
        <!-- 固定ヘッダー -->
        <div
          class="sticky top-0 bg-white border-b border-gray-200 z-10 px-4 py-3 flex items-center justify-between"
        >
          <button
            @click="closePhotoTour"
        ...
        <div class="lg:col-span-2 space-y-10">
          ...
          <section class="pb-8 border-b border-gray-200">
            ...
            <p class="text-base text-gray-700 mb-4" style="line-height: 1.8">
              世界的建築家・坂茂が1995年に設計した「家具の家
              No.1」。家具そのものが構造体となる革新的な建築に、実際に宿泊できる貴重な体験です。
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <component
                  ...
                  class="w-6 h-6 text-gray-700 flex-shrink-0 mt-0.5"
                />
                <div>
                  <p class="font-medium text-gray-900">{{ amenity.name }}</p>
                  <p v-if="amenity.description" class="text-sm text-gray-600">
                    {{ amenity.description }}
                  </p>
                </div>
              </div>
          ...
          >
            <div
              @click.stop
              class="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto m-4"
            >
              <!-- モーダルヘッダー -->
              <div
                class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between"
              >
                <h3 class="text-xl font-medium" style="color: #231815">
                  提供設備・アメニティ
                </h3>
                <button
                  @click="showAllAmenities = false"
                ...
                >
                  ...
                  <div class="space-y-4">
                    ...
                    >
                      <component
                        :is="getIconComponent(amenity.icon)"
                        class="w-7 h-7 text-gray-700 flex-shrink-0"
                      />
                      <div class="flex-1">
            ...
            <div class="flex items-center justify-between mb-6">
              <div class="flex items-center gap-3">
                ...
                <span class="text-gray-600"
                  >（{{ approvedReviews.length }}件のレビュー）</span
                >
              </div>
              <NuxtLink
                v-if="user"
            ...
            <div class="border border-gray-300 rounded-xl p-6 shadow-lg">
              <div class="mb-6">
                <div class="flex items-baseline gap-2 mb-2">
                  <span class="text-2xl font-medium" style="color: #231815"
              ...
              <div class="border border-gray-300 rounded-lg mb-4 relative">
                ...
                <div class="grid grid-cols-2 border-b border-gray-300">
                  ...
                  >
                    ...
                    <span class="text-sm text-gray-700">{{
                      checkInDate ? formatDate(checkInDate) : "日付を選択"
                    }}</span>
                  </button>
                  <button
                    @click="showCalendar = !showCalendar"
                  ...
                  >
                    ...
                    >
                    <span class="text-sm text-gray-700">{{
                      checkOutDate ? formatDate(checkOutDate) : "日付を選択"
                    }}</span>
                  </button>
                </div>
                ...
                <div
                  v-if="showCalendar"
                  class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-25 p-4"
                  @click="showCalendar = false"
                >
                  <div
                    @click.stop
                    class="bg-white rounded-xl shadow-2xl border border-gray-200 p-4 md:p-6 w-full max-w-[340px] md:max-w-[700px]"
                  >
                    <!-- カレンダーヘッダー -->
                    <div class="flex items-center justify-between mb-4 md:mb-6">
                      ...
                      <div class="flex gap-4 md:gap-12">
                        ...
                        <span
                          class="font-semibold text-sm md:text-base hidden md:block"
                          >{{ nextMonthName }}</span
                        >
                      </div>
                      <button
                    ...
                    <!-- カレンダーフッター -->
                    <div
                      class="flex items-center justify-between mt-4 md:mt-6 pt-4 md:pt-6 border-t border-gray-200"
                    >
                      <button
                        @click="clearDates"
              ...
              <div class="space-y-3 text-sm text-gray-700">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-gray-600 mt-1">
                      東京から約2時間<br />中央道 山中湖ICより15分
                    </p>
                  </div>
                </div>
                <NuxtLink
                  to="/access"
                  class="text-sm font-medium underline hover:text-gray-600 mt-2 transition-colors inline-block"
                >
                  アクセス詳細を見る
                </NuxtLink>
              </div>
      ...
      <section class="mt-16 pt-8 border-t border-gray-200">
        <h2
          class="text-2xl font-medium mb-8 text-center"
          style="color: #231815; font-weight: 500"
        >
          施設情報
        </h2>
      ...
      <section class="mt-16 pt-8 border-t border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <!-- キャンセルポリシー -->
          <div>
            <h3 class="text-base font-medium mb-4" style="color: #231815">
              キャンセルポリシー
            </h3>
            <div class="space-y-3 text-sm text-gray-700">
      ...
      <!-- CTAセクション -->
      <section
        class="mt-16 mb-12 py-12 px-8 rounded-2xl text-center"
        style="background: linear-gradient(135deg, #f8f7f5 0%, #ebe8e6 100%)"
      >
        <h2
        ...
        <p
          class="text-gray-700 mb-8 max-w-2xl mx-auto"
...
const displayPhotos = computed(() => {
  const photos =
    allPhotos.value.length > 0 ? [...allPhotos.value] : fallbackPhotos.value;
...
// 日付選択
const selectDate = (dateObj: CalendarDateObject) => {
...
const handleReservation = () => {
  ...
  if (isDateRangeUnavailable(checkIn, checkOut)) {
    toast.warning(
      "選択された期間には予約できない日が含まれています。別の日程をお選びください。",
    );
    return;
  }
  ...
  meta: [
    {
      name: "description",
      content:
        "世界的建築家・坂茂が設計した「家具の家 No.1」。1日1組限定で、この革新的な建築に宿泊できる貴重な体験をご提供します。山中湖の自然に囲まれた特別な時間をお過ごしください。",
    },
    { property: "og:title", content: "家具の家 No.1 | 宿泊予約" },
    {
      property: "og:description",
      content: "坂茂建築の傑作に宿泊。1日1組限定の特別な建築体験。",
    },
