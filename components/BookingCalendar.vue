        ...
        :class="[
          ...
          'focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2',
          {
            ...
            'hover:bg-gray-50 cursor-pointer':
              date.isAvailable && !date.isSelected,
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="font-semibold" aria-labelledby="checkin-label">
        ...
        <div v-if="selectedRange.end">
          <p class="text-sm text-gray-600" id="checkout-label">
            チェックアウト
          </p>
          <p class="font-semibold" aria-labelledby="checkout-label">
      ...
      >
        ...
        <p class="font-semibold" aria-labelledby="nights-label">
          {{ numberOfNights }}泊
        </p>
      </div>
    </div>
...
const { loadBlockedDates, loadBookedDates, isDateBlocked, isDateBooked } =
  useBlockedDates();
...
onMounted(async () => {
  await Promise.all([loadBlockedDates(), loadBookedDates()]);
...
const numberOfNights = computed(() => {
  ...
  const diffTime =
    selectedRange.value.end.getTime() - selectedRange.value.start.getTime();
