<template>
  <div class="space-y-6">
    ...
    <div class="card">
      ...
      <div class="mb-4 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          >
          <select
            v-model="rateFilter.nightCategory"
            class="w-full px-3 py-2 border rounded-lg text-sm"
          >
            <option value="">すべて</option>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-4 py-3 text-sm">
                {{ getNightLabel(rate.nightCategory) }}
              </td>
              <td class="px-4 py-3 text-sm">
                {{ rate.dayType === "weekday" ? "平日" : "休日前日" }}
              </td>
    ...
    <div class="flex justify-end gap-4">
      ...
      <button @click="savePricingSettings" class="btn-primary">保存する</button>
    </div>
  </div>
</template>
...
// 料金設定（編集可能）
const pricingSettings = reactive<DetailedPricingSetting>(
  createDemoPricingSetting(),
);
...
const filteredRates = computed(() => {
  return pricingSettings.priceRates.filter((rate) => {
    if (rateFilter.seasonType && rate.seasonType !== rateFilter.seasonType)
      return false;
    if (
      rateFilter.nightCategory &&
      rate.nightCategory !== rateFilter.nightCategory
    )
      return false;
    if (rateFilter.dayType && rate.dayType !== rateFilter.dayType) return false;
    if (
      rateFilter.guestCount &&
      rate.guestCount !== Number(rateFilter.guestCount)
    )
      return false;
    return true;
  });
...
function removeRate(index: number) {
  const actualIndex = pricingSettings.priceRates.findIndex(
    (r) => filteredRates.value[index] === r,
  );
...
function generateAllRates() {
  if (
    !confirm(
      "すべての料金レートを自動生成しますか？既存のレートは上書きされます。",
    )
  ) {
    return;
  }
