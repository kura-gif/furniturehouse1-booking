  ...
  <div class="card">
    ...
    <div v-else>
      ...
      <div class="mb-8 p-4 bg-purple-50 rounded-lg border border-purple-200">
        <h3 class="font-semibold text-purple-900 mb-2">現在のポリシー: {{ currentPolicy.name }}</h3>
        <p class="text-sm text-purple-700 whitespace-pre-line">{{ policyDescription }}</p>
      </div>
      ...
      <form @submit.prevent="handleSave" class="space-y-6">
        ...
        <div>
          ...
          <div class="space-y-3">
            ...
            >
              <div class="flex-1 grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs text-gray-600 mb-1">チェックイン何日前まで</label>
                  <input
                ...
                <div>
                  <label class="block text-xs text-gray-600 mb-1">返金率 (%)</label>
                  <input
          ...
          <p class="text-xs text-gray-500 mt-2">
            ※ルールは日数の大きい順に適用されます。例: 5日前まで100%、3日前まで50%、0日前（当日）まで0%
          </p>
        </div>
        ...
        <div class="p-4 bg-gray-50 rounded-lg">
          ...
          <p class="text-sm text-gray-600 whitespace-pre-line">{{ previewDescription }}</p>
        </div>
        ...
        <div class="p-4 border rounded-lg">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label class="block text-xs text-gray-600 mb-1">チェックイン日</label>
              <input
            ...
            <div class="flex items-end">
              <div v-if="simResult" class="p-3 bg-purple-50 rounded-lg w-full">
                <p class="text-xs text-gray-600">チェックイン{{ simResult.daysBeforeCheckIn }}日前</p>
...
// ポリシー説明文
const policyDescription = computed(() => generatePolicyDescription(currentPolicy.value.rules))
const previewDescription = computed(() => generatePolicyDescription(editPolicy.value.rules))
...
const handleSave = async () => {
  ...
  try {
    // ルールを日数の降順でソート
    editPolicy.value.rules.sort((a, b) => b.daysBeforeCheckIn - a.daysBeforeCheckIn)
