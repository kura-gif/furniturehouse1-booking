<template>
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <div
      v-else-if="localPhotos.length === 0"
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      >
        ...
        <div class="absolute top-2 left-2 flex gap-2">
          <span
            class="px-2 py-1 bg-gray-900 bg-opacity-75 text-white text-xs rounded"
          >
    ...
    >
      <div
        @click.stop
        class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-4"
      >
        <div
          class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between"
        >
          <h3 class="text-xl font-medium">
            {{ editingPhoto ? "写真を編集" : "写真を追加" }}
          </h3>
          <button
            @click="closeModal"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              ...
              <span class="text-xs text-gray-500 ml-2"
                >（Imgur、Cloudinary、Googleドライブなどの画像URLを入力）</span
              >
            </label>
            <input
...
const savePhoto = async () => {
  ...
  try {
    if (editingPhoto.value) {
      ...
      const index = localPhotos.value.findIndex(
        (p) => p.id === editingPhoto.value!.id,
      );
      if (index !== -1) {
