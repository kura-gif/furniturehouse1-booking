<template>
  <div class="bg-white rounded-lg shadow p-6">
    ...
    <div v-else-if="localAmenities.length === 0" class="text-center py-12 border-2 border-dashed border-gray-300 rounded-lg">
      ...
      >
        <div class="flex items-center gap-4 flex-1">
          <div class="flex items-center gap-3">
            ...
            />
            <component :is="getIconComponent(amenity.icon)" class="w-6 h-6 text-gray-600" />
          </div>
          <div class="flex-1">
            <div class="flex items-center gap-3">
            ...
            <p v-if="amenity.description" class="text-sm text-gray-500 mt-1">{{ amenity.description }}</p>
          </div>
        </div>
    ...
    <!-- 追加/編集モーダル -->
    <div v-if="showAddModal || editingAmenity" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" @click="closeModal">
      <div @click.stop class="bg-white rounded-xl shadow-2xl max-w-2xl w-full m-4 p-6">
        ...
        <form @submit.prevent="saveAmenity" class="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            >
              <option v-for="(label, key) in amenityCategories" :key="key" :value="key">
                {{ label }}
              </option>
            </select>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              ...
              >
                <div class="flex items-center gap-2">
                  <component :is="getIconComponent(formData.icon)" class="w-5 h-5 text-gray-600" />
              ...
              >
                ...
                >
                  <component :is="getIconComponent(icon)" class="w-5 h-5 text-gray-600 flex-shrink-0" />
          ...
          <div class="flex items-center gap-2">
            <input
              ...
              class="w-5 h-5 text-indigo-600 rounded"
            />
            <label for="available" class="text-sm font-medium text-gray-700">トップページに表示</label>
          </div>
...
import { amenityCategories, amenities as defaultAmenities, type Amenity } from '~/config/amenities'
import {
  WifiIcon,
  TruckIcon,
  SunIcon,
...
const filteredAmenities = computed(() => {
  ...
  return localAmenities.value.filter(a => a.category === selectedCategory.value)
})
...
const saveAmenity = async () => {
  try {
    if (editingAmenity.value) {
      ...
      const index = localAmenities.value.findIndex(a => a.id === editingAmenity.value!.id)
      if (index !== -1) {
...
const initializeDefaultAmenities = async () => {
  if (!confirm('初期データを投入しますか？既存のデータがある場合は追加されます。')) {
    return
  }
