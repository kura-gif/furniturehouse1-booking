  ...
  <div class="min-h-screen bg-gray-50">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex items-center gap-2">
                ...
                <span class="font-medium">{{
                  appUser?.displayName || user.displayName || "ゲスト"
                }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <div class="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p class="text-xs text-gray-500">
                      予約番号: {{ booking.bookingReference || booking.id }}
                    </p>
                    <!-- 却下理由の表示 -->
                    <p
                      v-if="
                ...
                <div class="grid md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p class="font-semibold">
                      {{
                        formatDateWithDay(
                          (booking.startDate ?? booking.checkInDate).toDate(),
                        )
                      }}
                    </p>
                    <p class="text-xs text-gray-500">
                      {{ siteSettings.checkInTime }}〜
                    </p>
                  </div>
                  <div>
                    ...
                    <p class="font-semibold">
                      {{
                        formatDateWithDay(
                          (booking.endDate ?? booking.checkOutDate).toDate(),
                        )
                      }}
                    </p>
                    <p class="text-xs text-gray-500">
                      〜{{ siteSettings.checkOutTime }}
                    </p>
                  </div>
                  <div>
                ...
                <div class="mt-3 pt-3 border-t border-gray-100">
                  <div class="flex justify-between items-center">
                    ...
                    <span class="font-bold text-lg text-purple-700"
                      >¥{{ (booking.totalAmount ?? 0).toLocaleString() }}</span
                    >
                  </div>
                </div>
    ...
    >
      <div class="bg-white rounded-xl p-6 max-w-lg w-full" @click.stop>
        ...
        <div v-else-if="refundCalculation">
              ...
              <div>
                ...
                <p class="font-medium">
                  {{
                    formatDate(
                      (
                        cancelTargetBooking.startDate ??
                        cancelTargetBooking.checkInDate
                      ).toDate(),
                    )
                  }}
                </p>
              ...
              <div>
                ...
                <p class="font-medium">
                  {{
                    formatDate(
                      (
                        cancelTargetBooking.endDate ??
                        cancelTargetBooking.checkOutDate
                      ).toDate(),
                    )
                  }}
                </p>
          ...
          <!-- キャンセルポリシー情報 -->
          <div
            class="border border-purple-200 bg-purple-50 rounded-lg p-4 mb-4"
          >
            <h4 class="font-medium text-purple-900 mb-2">キャンセルポリシー</h4>
            <p class="text-sm text-purple-800 mb-3">
              チェックインまであと
              <span class="font-bold"
                >{{ refundCalculation.daysBeforeCheckIn }}日</span
              >
            </p>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-medium"
                  >¥{{
                    refundCalculation.originalAmount.toLocaleString()
                  }}</span
                >
              </div>
              <div class="flex justify-between">
              ...
              <div
                v-if="refundCalculation.nonRefundableAmount > 0"
                class="flex justify-between text-red-600"
              >
                <span>返金対象外</span>
                <span
                  >¥{{
                    refundCalculation.nonRefundableAmount.toLocaleString()
                  }}</span
                >
              </div>
            </div>
          ...
          >
            <p class="text-sm text-yellow-800">
              <strong>注意:</strong>
              キャンセル後の取り消しはできません。返金処理には数日かかる場合があります。
            </p>
          </div>
    ...
    >
      ...
      >
        ...
        <div
          v-if="cancelSuccessRefundAmount > 0"
          class="bg-green-50 rounded-lg p-4 mb-6"
        >
          <p class="text-sm text-gray-600 mb-1">返金予定額</p>
          <p class="text-2xl font-bold text-green-600">
            ¥{{ cancelSuccessRefundAmount.toLocaleString() }}
          </p>
...
const loadBookings = async () => {
  ...
  try {
    ...
    for (const booking of allBookings) {
      ...
      if (
        checkInDate >= today ||
        ["pending", "pending_review", "confirmed"].includes(booking.status)
      ) {
    ...
    upcoming.sort(
      (a, b) =>
        (a.startDate ?? a.checkInDate).toDate().getTime() -
        (b.startDate ?? b.checkInDate).toDate().getTime(),
    );
    past.sort(
      (a, b) =>
        (b.startDate ?? b.checkInDate).toDate().getTime() -
...
const canMessage = (status: string) => {
  return !["cancelled", "rejected", "expired", "payment_failed"].includes(
    status,
  );
};
...
const handleCancelBooking = async (booking: Booking) => {
  ...
  try {
    ...
    cancelError.value =
      fetchError.data?.message || "返金額の計算に失敗しました";
  } finally {
...
const confirmCancelBooking = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      cancelSuccessRefundAmount.value =
        refundInfo?.processed && refundInfo?.amount > 0 ? refundInfo.amount : 0;
      closeCancelModal();
    ...
    const fetchError = error as { data?: { message?: string } };
    cancelError.value =
      fetchError.data?.message || "キャンセル処理に失敗しました";
  } finally {
