    ...
    <div class="container-responsive py-8">
      ...
      <div class="card mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select v-model="selectedYear" class="input-field" @change="loadReport">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">月</label>
            <select v-model="selectedMonth" class="input-field" @change="loadReport">
          ...
          <div>
            ...
            <select v-model="selectedSupporterId" class="input-field" @change="loadReport">
          ...
          <div class="flex items-end">
            <button @click="exportCsv" :disabled="!reportData.length" class="btn-primary w-full">
              CSVエクスポート
            </button>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div class="card">
          ...
          <p class="text-3xl font-bold text-blue-600">{{ summary.totalTasks }}</p>
        </div>
        <div class="card">
      ...
      <div class="card mb-6">
        ...
        <div v-else-if="supporterSummaries.length === 0" class="text-center text-gray-500 py-8">
          該当するデータがありません
        </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                    :class="[
                      ...
                      s.unpaidCount === 0 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    ]"
                  >
                    {{ s.unpaidCount === 0 ? '全て支払い済み' : `${s.unpaidCount}件未払い` }}
                  </span>
                </td>
              </tr>
            </tbody>
      ...
      <div class="card">
        ...
        <div v-if="reportData.length === 0 && !loading" class="text-center text-gray-500 py-8">
          該当するデータがありません
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="task in reportData" :key="task.id">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm">
                  {{ task.actualHours ? task.actualHours.toFixed(2) + '時間' : '-' }}
                </td>
                <td class="px-4 py-3 whitespace-nowrap text-sm">
                ...
                <td class="px-4 py-3 whitespace-nowrap">
                  <span
                    :class="[
                      'px-2 py-1 rounded-full text-xs',
                      task.compensation?.isPaid ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    ]"
                  >
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm">
                  <button
                    v-if="task.status === 'completed' && !task.compensation?.isPaid"
        ...
        <div v-if="unpaidTasks.length > 0" class="mt-6 pt-6 border-t">
          <div class="flex items-center justify-between">
            <p class="text-gray-600">
              未払いタスク: <strong>{{ unpaidTasks.length }}件</strong>
              （合計 <strong>¥{{ unpaidTotalAmount.toLocaleString() }}</strong>）
            </p>
...
const availableYears = computed(() => {
  const years = []
  for (let y = currentDate.getFullYear() - 3; y <= currentDate.getFullYear() + 1; y++) {
    years.push(y)
  }
  return years
})
...
const summary = computed(() => {
  const completedTasks = reportData.value.filter(t => t.status === 'completed')
  return {
    totalTasks: completedTasks.length,
    totalHours: completedTasks.reduce((sum, t) => sum + (t.actualHours || 0), 0),
    totalCompensation: completedTasks.reduce((sum, t) => sum + (t.compensation?.calculatedAmount || 0), 0),
...
const supporterSummaries = computed<SupporterSummary[]>(() => {
  ...
  return Array.from(summaryMap.values()).sort((a, b) => b.totalCompensation - a.totalCompensation)
})

// 未払いタスク
const unpaidTasks = computed(() =>
  reportData.value.filter(t => t.status === 'completed' && !t.compensation?.isPaid)
)

const unpaidTotalAmount = computed(() =>
  unpaidTasks.value.reduce((sum, t) => sum + (t.compensation?.calculatedAmount || 0), 0)
)
...
const exportCsv = () => {
  ...
  rows.push(['総報酬額', '¥' + summary.value.totalCompensation.toLocaleString()])
  rows.push(['未払い額', '¥' + summary.value.unpaidAmount.toLocaleString()])

  // サポーター別サマリー
  rows.push([])
  rows.push(['--- サポーター別サマリー ---'])
  rows.push(['サポーター名', 'タスク数', '作業時間', '時給', '交通費合計', '報酬合計', '未払い件数'])
