<template>
  <div class="min-h-screen bg-gray-50">
        ...
        <form @submit.prevent="submitAssignment" class="space-y-4">
          <div>
            ...
            <div class="bg-gray-50 p-3 rounded-lg text-sm">
              <p>
                <strong>予約ID:</strong> {{ assigningTask.bookingReference }}
              </p>
              <p>
                <strong>タイプ:</strong>
                {{
                  (assigningTask.taskType || assigningTask.type) ===
                  "pre_checkin"
                    ? "チェックイン前"
                    : "チェックアウト後"
                }}
              ...
              <p>
                <strong>予定日:</strong>
                {{ formatDate(assigningTask.scheduledDate) }}
              </p>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              {{ assigningTask.assignedTo ? "新しいサポーター" : "サポーター" }}
            ...
            <select v-model="selectedSupporterId" required class="input-field">
              ...
              <option
                ...
                :value="supporter.id"
              >
                {{ supporter.displayName }} (時給: ¥{{
                  supporter.hourlyRate?.toLocaleString() || 0
                }})
              </option>
            </select>
          </div>
          ...
          <div class="flex gap-3 pt-4">
            <button
              type="submit"
              :disabled="!selectedSupporterId || assigning"
              class="btn-primary flex-1"
            >
              {{
                assigning
                  ? "処理中..."
                  : assigningTask.assignedTo
                    ? "変更する"
                    : "割り当て"
              }}
            </button>
    ...
    >
      <div
        class="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 my-8"
        @click.stop
      >
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p class="font-semibold">
                {{
                  (viewingTask.taskType || viewingTask.type) === "pre_checkin"
                    ? "チェックイン前"
                    : "チェックアウト後"
                }}
              </p>
            ...
            <div>
              ...
              <p class="font-semibold">
                {{ formatDate(viewingTask.scheduledDate) }}
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p class="font-semibold">
                {{ viewingTask.assignedToName || "未割当" }}
              </p>
            </div>
            <div>
          ...
          <!-- 作業時間と報酬 -->
          <div
            v-if="viewingTask.startedAt || viewingTask.completedAt"
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <p>
                  {{
                    viewingTask.startedAt
                      ? formatDateTime(viewingTask.startedAt)
                      : "-"
                  }}
                </p>
              </div>
                ...
                <p>
                  {{
                    viewingTask.completedAt
                      ? formatDateTime(viewingTask.completedAt)
                      : "-"
                  }}
                </p>
              </div>
                ...
                <p>
                  {{
                    viewingTask.actualHours
                      ? viewingTask.actualHours.toFixed(2) + "時間"
                      : "-"
                  }}
                </p>
              </div>
              <div>
                ...
                <p class="font-semibold text-lg">
                  ¥{{
                    viewingTask.compensation?.calculatedAmount?.toLocaleString() ||
                    0
                  }}
                </p>
              </div>
              <div>
                ...
                <p>
                  <span
                    :class="[
                      'px-2 py-1 rounded-full text-xs',
                      viewingTask.compensation?.isPaid
          ...
          <div v-if="viewingTask.checklist?.length > 0">
            ...
            <div class="bg-gray-50 p-4 rounded-lg space-y-2">
              <div
                v-for="(item, index) in viewingTask.checklist"
                :key="index"
              ...
              >
                ...
                <span v-if="item.isCustom" class="text-xs text-purple-600"
                  >(カスタム)</span
                >
              </div>
            </div>
          ...
          <div v-if="(viewingTask.suppliesUsed?.length ?? 0) > 0">
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
              <div
                v-for="supply in viewingTask.suppliesUsed"
                :key="supply.name"
                class="text-sm"
              >
          ...
          <div v-if="viewingTask.notes">
            ...
            <p class="bg-gray-50 p-3 rounded-lg text-sm whitespace-pre-wrap">
              {{ viewingTask.notes }}
            </p>
          </div>
          ...
          <div v-if="viewingTask.adminNotes">
            ...
            <p class="bg-yellow-50 p-3 rounded-lg text-sm whitespace-pre-wrap">
              {{ viewingTask.adminNotes }}
            </p>
          </div>
...
// アクティブなサポーターのみ
const activeSupporters = computed(() =>
  supporters.value.filter((s) => s.isActive !== false),
);
...
const submitAssignment = async () => {
  ...
  try {
    const supporter = supporters.value.find(
      (s) => s.id === selectedSupporterId.value,
    );
    if (!supporter) throw new Error("サポーターが見つかりません");

    await assignTask(
      assigningTask.value.id,
      supporter.id,
      supporter.displayName,
    );
    ...
    toast.success("サポーターを割り当てました（通知メールを送信しました）");
    closeAssignModal();
    await loadAllTasks();
  } catch (error) {
    console.error("❌ 割り当てエラー:", error);
    toast.error("サポーターの割り当てに失敗しました");
  } finally {
...
const removeAssignment = async (task: CleaningTask) => {
  if (
    !(await confirmDialog.confirm({
      title: "割り当て解除",
      message: `${task.assignedToName} さんの割り当てを解除しますか？\nタスクは「未割当」状態に戻ります。`,
      type: "warning",
    }))
  ) {
    return;
