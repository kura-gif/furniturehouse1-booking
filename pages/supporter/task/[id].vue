  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="container-responsive py-8">
      ...
      <div v-else-if="!task" class="card text-center py-12">
        ...
        <NuxtLink to="/supporter" class="btn-primary"
          >ダッシュボードへ戻る</NuxtLink
        >
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        >
          ...
          <div
            v-if="task.status === 'in_progress' && task.startTime"
            class="text-right"
          >
        ...
        <div class="card">
          ...
          <div class="mt-4 text-sm text-gray-600">
            完了: {{ completedChecklistCount }} /
            {{ task.checklist?.length || 0 }} 項目
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div v-else>
            <div
              v-if="!task.suppliesUsed || task.suppliesUsed.length === 0"
              class="text-gray-500"
            >
              備品の使用記録はありません
            </div>
            <ul v-else class="space-y-2">
              <li
                v-for="(supply, index) in task.suppliesUsed"
                :key="index"
        ...
        <!-- 報酬情報（完了時のみ） -->
        <div
          v-if="task.status === 'completed' && task.compensation"
          ...
          <div class="space-y-3">
            <div class="flex justify-between">
              ...
              <span class="font-semibold"
                >{{ task.compensation.hoursWorked?.toFixed(2) }}時間</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-green-700">時給</span>
              <span class="font-semibold"
                >¥{{ task.compensation.hourlyRate?.toLocaleString() }}</span
              >
            </div>
            <div class="flex justify-between">
              ...
              <span class="font-semibold">
                ¥{{
                  Math.round(
                    (task.compensation.hoursWorked || 0) *
            ...
            <div class="flex justify-between">
              ...
              <span class="font-semibold"
                >¥{{
                  task.compensation.transportationFee?.toLocaleString()
                }}</span
              >
            </div>
            ...
            <div class="flex justify-between text-lg">
              ...
              <span class="font-bold text-green-600"
                >¥{{ task.compensation.totalAmount?.toLocaleString() }}</span
              >
            </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-semibold"
              ...
              {{ task?.checklist?.length || 0 }} 完了</span
            >
          </div>
        </div>
...
const loadTask = async () => {
  ...
  try {
    ...
    if (task.value) {
      ...
      supplies.value = task.value.suppliesUsed
        ? [...task.value.suppliesUsed]
        : [];
...
const startTask = async () => {
  ...
  try {
    ...
    task.value.startTime = Timestamp.fromDate(startTime);
    startElapsedTimer();
  } catch (error) {
...
const completeTask = async () => {
  ...
  try {
    ...
    task.value.endTime = Timestamp.fromDate(endTime);
...
// フォーマット関数
function formatDate(timestamp: TimestampLike): string {
  if (!timestamp) return "-";
  try {
    const date =
